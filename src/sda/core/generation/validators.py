...
def _build_address_for_city(city: str, *, locale: str, seeded_random: Random) -> str:
    street_lines = STREET_LINES.get(locale, STREET_LINES["en_US"])
    districts = CITY_DISTRICTS.get(locale, CITY_DISTRICTS["en_US"])
    street_line = seeded_random.choice(street_lines)
    ...
    if locale == "ru_RU":
        ...
        patterns = [
            f"{street_line}, д. {house}{building}{apartment}",
            f"{district} район, {street_line}, д. {house}{building}{apartment}",
            f"{street_line}, влад. {house}{building}{apartment}",
        ]
        return _normalize_address_text(seeded_random.choice(patterns))

    district = seeded_random.choice(districts)
    suite = f", Apt {seeded_random.randint(1, 999)}" if seeded_random.random() < 0.55 else ""
    patterns = [
        f"{house} {street_line}{suite}",
        f"{house} {street_line}, {district}{suite}",
        f"{house} {street_line}, Unit {seeded_random.randint(1, 80)}",
    ]
    return _normalize_address_text(seeded_random.choice(patterns))
